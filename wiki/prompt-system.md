...
relations:
  ...
  - target: "[[tool-system]]"
    type: depends_on
    evidence: "AgentScope 2.x formatter 负责将 ToolCall/ToolResult/DataBlock 投影到 provider API；tool result 多模态提升和不支持 media 的本地/URL fallback 都要求 prompt-system 感知 tool-system 的结果结构"
...
| ---- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| 核心设计 | 分层认知控制框架，通过 `buildEffectiveSystemPrompt()` 按优先级动态装配六层 prompt，将 prompt 从静态模板升格为运行时策略层                                | 通过 `build_runtime_system_prompt()` 在每次 `submit_message()` 前同步重建完整 system prompt，按固定顺序拼接七个独立 section（环境信息、推理设置、skills 索引、CLAUDE.md、issue/PR 上下文、MEMORY.md、记忆文件）       | 分段组装（基础指令 + `<skill_system>` 技能索引 + `<memory>` 记忆注入 + 可选 SOUL.md 人格层），技能懒加载——提示里只放技能名/描述索引，任务匹配时再按需加载完整 SKILL.md，大幅压缩默认提示 token 占用 | **稳定缓存前缀 + 动态 ephemeral 追加**：`_build_system_prompt()` 在 session 开始时一次性构建 9 层装配流水线并缓存，每轮追加 ephemeral 层和 per-turn 记忆注入（注入到 messages 流而非 system prompt），系统 prompt 前缀全程不变，prefix cache 命中率接近 100% | System prompt builder + provider formatter 双层：`Agent._prepare_model_input()` 组装 system prompt、summary、context、tools、skill instructions；`FormatterBase` 及 provider-specific formatter 负责把内部 `Msg` / block 转为目标 API 格式 |
| 关键特点 | 静态/动态 section 分离 + `SYSTEM_PROMPT_DYNAMIC_BOUNDARY` 缓存感知设计；失败模式前置规则（伪完成/越权/过度设计压制）；coordinator 模式独立 prompt 实现完整认知切换 | `effort` + `passes` 作为 `# Reasoning Settings` section 注入 prompt，轻量替代 extended thinking；CLAUDE.md 向上目录遍历 + `.claude/rules/*.md` 支持全局与项目级规则叠加；记忆检索采用启发式 token 匹配，无外部依赖 | 技能懒加载：技能数量与提示 token 开销完全解耦；SOUL.md 人格层：人格与逻辑指令解耦，自定义 agent 可替换人格；tiktoken 精确预算：记忆注入用精确 token 计数而非字符估算                              | per-model 执行协议：针对通用/OpenAI(GPT)/Google(Gemini) 三套独立执行纪律，靶向压制各家模型已知失败模式；SOUL.md persona-as-file：agent 身份做成可替换文件，runtime 不改代码即可托管完全不同人格；三层注入防护（内容正则 + 路径黑名单 + 注入量限制）                            | `on_system_prompt` middleware 可按 session 顺序改写 system prompt；MultiAgentFormatter 把多 agent 历史投影到 provider 可接受格式；`FormatterBase.convert_tool_result_to_string()` 处理不支持的多模态 tool result，支持提升为 user-side data block 或写入 URL/本地路径提醒 |
| 局限   | 动态 section 持续膨胀，跨 section 语义冲突无工具检测；工具 prompt 与主 prompt 之间缺乏显式语义协调机制                                                | 无 prompt caching，每轮完整重建产生重复 token 费用；同步重建在记忆文件较多时可能成为延迟瓶颈                                                                                                            | 无提示缓存边界优化（每轮全量重新计算）；无推理提示注入（无 effort/passes 控制）；技能懒加载每次 read_file 消耗一个工具调用轮次                                                       | Skin 视觉主题与 SOUL.md 人格两套系统各自独立，无同步机制，可能产生 banner 与 prompt 人格不一致；context files 取一不合并（`.hermes.md` > `AGENTS.md` > `CLAUDE.md` > `.cursorrules`），多规范共存时低优先级文件被静默忽略；注入防护为正则匹配，语义等价绕过无法识别          | Formatter 不再承担自动截断，token 预算由 agent/model 层处理；MultiAgentFormatter 历史压缩损失消息边界；Base64 fallback 产生本地临时文件，分布式部署需换成 workspace/object-store URI；capability 过滤不会自动路由替代模型 |
...
- **多 agent 对话 / 生产级 provider 多样化部署** → 外置 Formatter 层（AgentScope 方案）。把 provider 差异、多 agent 消息兼容、多模态 tool result 投影都收敛到 formatter，而不是散落在 agent loop。AgentScope 2.x 的 `FormatterBase` 不负责自动截断；token 预算和 context compression 应由 agent/model/context manager 层处理。
...
7. **把 formatter 当 context manager 用**：formatter 只能保证 provider API 格式，不应承担“哪些历史该保留”的业务决策。AgentScope 2.x 已把压缩放回 `Agent.compress_context()`；agent-os 也应把 `MessageFormatter` 与 `ContextManager` 分开。

## 相关模式

- [[formatter-as-provider-boundary]] — 将 provider 差异、多 agent 消息兼容和多模态 tool result 投影收敛到 formatter 层；context 压缩另由 agent/model 层处理
