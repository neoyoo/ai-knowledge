...
relations:
  ...
  - target: "[[tool-system]]"
    type: depends_on
    evidence: "AgentScope 2.x 的 `MiddlewareBase.on_acting` 只包裹纯工具 I/O 执行层，权限检查和 context 写入在 hook 外部完成；hooks 的安全边界依赖 tool system 的执行分层"
...
| Middleware Hook（运行时链） | 用 middleware 包裹 reply/model/tool/context 等生命周期，按 session 动态装配 | Web/distributed agent runtime、需要按租户或 session 注入策略/观测 | AgentScope 2.x |
...
**如果是 Web/distributed agent runtime，需要按 session 注入策略或观测 → Middleware Hook**

AgentScope 2.x 的做法是：Agent 初始化时把 middleware 按 hook 类型拆成多个链，`on_reply`、`on_reasoning`、`on_model_call`、`on_acting` 和 `on_compress_context` 走 onion pattern，`on_system_prompt` 走 transformer pattern。适合以下场景：

- 同一服务中不同用户/session 需要不同审计、策略、inbox 或 offload 逻辑
- 工具执行希望把纯 I/O 与权限/context 写入分开，降低后台执行的状态竞态
- system prompt 需要可组合的运行时改写，但不想把全部逻辑塞进 prompt builder

代码证据：`src/agentscope/middleware/_base.py` 与 `src/agentscope/agent/_agent.py`。
