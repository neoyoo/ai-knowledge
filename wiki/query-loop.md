...
relations:
  ...
  - target: "[[hooks]]"
    type: uses
    evidence: "DeerFlow middleware hook 与 AgentScope 2.x middleware 均直接介入 query-loop 的 reasoning、tool acting、model call、context compression 等生命周期点"
  - target: "[[channel-remote]]"
    type: supports
    evidence: "query-loop 的流式输出、中断与异步生成器是 SSE / realtime channel 暴露执行过程的基础"
  - target: "[[multi-agent]]"
    type: depends_on
    evidence: "AgentScope 2.x 的 team worker 通过独立 session/inbox/wakeup 复用 query-loop；多 agent 拓扑不再是旧 MsgHub 广播，而是 Web runtime 的 session/run 基础设施"
...
|------|------------|-------------|----------|-------------|-----------|
| 核心设计 | `query.ts` + `QueryEngine.ts` 双核分工：单回合执行器（状态机）+ 会话级控制器，将 agent turn 显式建模为可观测、可中断、可恢复的状态机 | `run_query()` 为核心，实现为最多 8 轮（可配置）的 `for` 循环：流式接收响应，有工具调用则执行（多工具通过 `asyncio.gather()` 并行），无工具调用则直接返回 | 循环本体委托给 LangGraph 的 `create_agent()`，自身创新集中在 14 个中间件组成的管道上，以 `before_agent`/`after_model`/`wrap_tool_call` 三类钩子介入每个关键节点，在不修改核心循环代码的前提下注入横切关注点 | `run_conversation()` 单一扁平循环（2400 行）：双条件守卫（`api_call_count < max_iterations AND iteration_budget.remaining > 0`）；所有逻辑（API 调用、工具执行、错误恢复、上下文压缩、预算管理）全压入同一方法体，核心哲学是**防御性工程** | 统一 `Agent.reply_stream()` / `reply()` 入口；`_reply_impl()` 驱动 event-first reasoning-acting loop；每轮先处理恢复事件或新消息，再 reasoning、batch tool calls、permission/external execution 暂停恢复；middleware 包裹 reply/reasoning/acting/model call/system prompt/context compression |
| 关键特点 | max_output_tokens 命中后自动降级重试（不直接失败）；工具结果写入前做 budget 控制（`applyToolResultBudget`）；REPL 与 SDK headless 模式共享同一 runtime | 单/多工具执行路径显式分支，代码意图一目了然；`asyncio.gather()` 并行执行多工具是 first-class 设计；`CostTracker` 内嵌于 `QueryEngine`，费用追踪随对话历史自然流动 | Loop Detection：滑动窗口（20 步）哈希检测，3 次触发警告、5 次强制中断循环；循环内 clarification：允许在 turn 执行中途暂停请求用户澄清；洋葱模型钩子顺序（before 正向 + after 逆向） | 9 个类型化回调（thinking/stream_delta/tool_start/tool_complete/step 等），回调异常不中断循环；`IterationBudget` 线程安全计数器，`execute_code` 工具免费（自动 refund）；路径级并行安全判定（同名工具写不同路径才并行）；预算压力注入工具结果 JSON（不发新消息） | 结构化 AgentEvent 流贯穿 model/text/thinking/tool/data/reply lifecycle；`UserConfirmResultEvent` / `ExternalExecutionResultEvent` 是 loop 原生输入；tool batch 按 `is_concurrency_safe` 决定串行/并发；tool result 在写 context 前截断/offload；Web `ChatService` 在 session lock 内消费 `reply_stream()` 并发布事件 |
| 局限 | 状态转换逻辑散落函数体中，无形式化状态转换图；固定 turnCount 上限不自适应任务复杂度 | 超过 `max_turns` 直接抛出 `RuntimeError`，无优雅降级；对话历史不做压缩，长对话可能撑爆 context window | 核心循环由 LangGraph 托管，无法控制低层级循环机制；中间件优先级由列表下标隐性决定；无推理深度控制（无 effort/passes 机制） | 单函数 2400 行，可测试性弱，边缘 case 行为只能读源码；父子 agent 预算独立不共享（90+5×50=340 实际上限远超预期）；工具结果无体积控制层（无 `applyToolResultBudget` 等价物） | 退出条件仍主要是“无 tool call”，无语义完成判断；concurrent batch 不表达工具依赖 DAG；确认/外部执行会暂停当前 run；`is_state_injected=True` 工具若被后台 offload 可能产生并发 state mutation 风险 |
...
### Event-first Loop 与 Channel 解耦

AgentScope 2.x 的 query loop 把输出统一成 `AgentEvent` 流，而不是直接面向 CLI 文本或 Web 响应。

- **做法**：`reply_stream()` 消费 `_reply_impl()`，产出 `ReplyStart/End`、`ModelCallStart/End`、Text/Thinking/Tool/Data block、ToolResult、RequireUserConfirm、RequireExternalExecution 等事件；`ChatService` 在 session lock 内消费事件并发布到 MessageBus，SSE channel 再做 replay/live fan-out。
- **与已有源对比**：OpenHarness 的 loop 更轻量，事件语义弱；Hermes Agent 有丰富回调但集中在单函数内；AgentScope 把事件作为 loop 的一等产物，更适合 Web/distributed runtime。
- **代码证据**：`src/agentscope/agent/_agent.py:191`、`:533`、`src/agentscope/app/_service/_chat.py:366`。

**场景建议**：agent-os 同时支持本地代理和 Web distributed agent 时，应让 loop 只产出结构化事件，本地 CLI、SSE、trace、review UI 分别消费事件，而不是各自嵌入一套 loop。
...
AgentScope 2.x 把用户确认和外部执行结果做成 query loop 的输入事件，而不是 channel 层的特殊分支。

- **做法**：工具调用经过 permission engine 后，可能产出 `RequireUserConfirmEvent`；外部工具会产出 `RequireExternalExecutionEvent`。loop 暂停后，下一次 `reply_stream()` 可用 `UserConfirmResultEvent` 或 `ExternalExecutionResultEvent` 恢复同一个 `reply_id` 和 tool call state。
- **与已有源对比**：Claude Code 也有结构化 permission relay；AgentScope 的特色是确认/外部执行结果直接进入 agent loop 输入类型，并由 session state 持久化承接。
- **代码证据**：`src/agentscope/agent/_agent.py:569`、`:882`、`:1437`、`:1487`。

**场景建议**：agent-os 的本地代理和 Web agent 都应把“用户确认 / 外部执行完成 / 后台任务完成”建模为可恢复输入事件。这样暂停、刷新、跨进程恢复都走同一条 loop 路径。
