...
把 provider API 差异、多 agent 消息兼容、tool_use/tool_result 配对和截断策略都收敛到独立 formatter 层，让 agent loop 只处理统一的内部消息对象。
...
- [[context-management]] — formatter 持有 token counter 和截断循环
...
Formatter 不只做字符串拼接，还负责在 token 超限时删除最旧完整 turn，并确保 `tool_use` 和 `tool_result` 成对保留或成对删除。
...
- 需要 provider-specific token counting / truncation
...
AgentScope formatter 体系：

- `wiki/_impl/prompt-system--agentscope.md` — ChatFormatter / MultiAgentFormatter / A2AChatFormatter
- `wiki/_impl/context-management--agentscope.md` — TruncatedFormatterBase 与 TokenCounterBase
- `wiki/_impl/channel-remote--agentscope.md` — realtime / A2A 消息桥接

## 迁移 checklist

- 定义 provider-neutral 的内部消息和 content block
- 每个 provider 单独实现 formatter，不在 query loop 写 `if provider == ...`
- 截断循环放在 formatter 公共基类
